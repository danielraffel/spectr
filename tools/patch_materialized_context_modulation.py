...
def make_adjacent_modulation_submenu(html):
    if 'data-spectr-modulation-anchor' in html:
        ...
        if 'const menuMaxHeight = Math.max(180, vh - 16);' in html:
            html = html.replace(
                'const menuMaxHeight = Math.max(180, vh - 16);',
                'const menuBottom = Math.max(24, vh - 64);\n'
                '  const menuMaxHeight = Math.max(120, menuBottom - 16);', 1)
        if 'Math.min(y, vh - H - 8)' in html:
            html = html.replace('Math.min(y, vh - H - 8)',
                                'Math.min(y, menuBottom - H - 8)', 1)
        if 'Math.min(top + 108 + (hasSel ? 100 : 0), vh - submenuHeight - 8)' in html:
            html = html.replace(
                'Math.min(top + 108 + (hasSel ? 100 : 0), vh - submenuHeight - 8)',
                'Math.min(top + 108 + (hasSel ? 100 : 0), menuBottom - submenuHeight - 8)', 1)
