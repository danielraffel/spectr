...
| Typography (`var(--mono)` / `var(--sans)`) | Inter / IBM Plex Mono shipped | Spectr ships ZERO fonts; piggy-backs on Pulp's accidentally-bundled Inter+JBM. Typography "works" only because tokens happen to resolve to those exact names. Two independent bugs cause visible drift. | **#1150** (no public font-registration API) + **#1151** (comma-separated fontFamily list passed verbatim) + **#1070** (drift symptoms) | none — needs `register_font_blob` + `pulp_add_fonts` CMake helper |
