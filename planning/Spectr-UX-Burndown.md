...
| DEL-3 | Logic AUv2 and REAPER VST3/CLAP acceptance | Formats implemented; installed baseline AU is `/Library/Audio/Plug-Ins/Components/Spectr.component` (`aufx/Spec/Pulp`) | `auval -v aufx Spec Pulp` passes. Installed user CLAP loads and passes fuzz/process/state checks but `clap-validator` reports the existing `Viewport Width` text-conversion contract failure (828/834); this is baseline-only, not new-package evidence. Logic and REAPER receipts remain pending | Pending | Open |
| DEL-4 | Signed, notarized, installed, launch-verified M5 PKG | Exact-current-head (`13dcc44`) four-payload Release build completed against official Pulp v0.829.0 SDK (`cc75fa91cf6942a197b2fb00b38ac679de3cbcd1`) | `artifacts/Spectr-1.0.1.pkg` SHA-256 `94e02b1b284c5cd79015824c037b7b10ed3d3d45e717c1f70003457e392faf8e`; Apple notarization submission `e0b2d8fb-2731-424c-8abe-7a1a9c606a29` accepted; staple, `spctl`, and installer signature validation pass | Install and launch confirmation still pending (non-interactive sudo unavailable) | Waiting human/host confirmation |
