...
| MOD-1 | Internal modulation/LFO Settings UI | Done | Materialized Chromium proof passes; both LFOs expose on/off, Sin/Tri/Square/Saw, tempo rate, depth, and bridge-backed hydration/write paths; native bridge test object compiles | Pending visual/audible check in new PKG | Waiting human confirmation |
| MOD-2 | Whole-bank, Snapshot A/B, and Morph targets | Implemented | Native target-mask bridge composes selected Bank/A/B/Morph destinations; focused unit and bridge all/none/error coverage added; exact-SDK/audio proof remains pending | Pending audible check in new PKG | Waiting automated proof + human confirmation |
| MOD-3 | Third-party host modulation works alongside internal modulation | Pulp successor foundation committed (`35364ef25f`, test extension `c757b68096`); Spectr now composes two native LFO overlays after host field/morph | Pulp modulation queue tests previously passed; Spectr target build reaches existing SDK incompatibility (`SpectralMaskProcessorT::set_layout_rt` absent) before link | Logic/REAPER after authoritative SDK/API lane | Open |
