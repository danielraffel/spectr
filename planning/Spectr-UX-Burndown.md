...
| SET-1 | Fixed header with content scrolling beneath | Done | Native frozen-state parity now passes 886 assertions, including body-only ScrollView topology and fixed-header geometry (`11731c6`) | Pending in new PKG | Waiting human confirmation |
...
| MOD-3 | Third-party host modulation works alongside internal modulation | Pulp successor foundation committed (`35364ef25f`, test extension `c757b68096`); Spectr composes two native LFO overlays after host field/morph | Pulp successor WidgetBridge sole-owner and pointer/gesture/capture dispatch contracts pass (ctest 14402, 14450); Spectr released-SDK suite passes 169 cases / 162,522 assertions; host coexistence receipt still absent | Logic/REAPER after the exact Pulp API is authoritative | Open |
