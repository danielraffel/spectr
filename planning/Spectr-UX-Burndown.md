...
| #8094 | Settings body collapsed (blank / ~50px) — the retained-scroll split cleared `flex_grow`/`flex_shrink` but left `flex_basis: 0` behind, so content main size resolved to `0 + padding` | MERGED 2026-09-07T03:00:09Z (`1d0030786`); an ancestor of the pinned SDK v0.846.0 and not of v0.835.0 |
...
product decision), SET-6 (unoracled, not blocked on geometry; SET-1 is closed — #8094 is in the pinned SDK), AUT-3
...
| SET-1 | Fixed header with content scrolling beneath | Done | UNBLOCKED — the 50px body viewport was real and is now fixed in the SDK, so the reading that blocked this row no longer reproduces. Pulp #8094 (`1d0030786`, the `flex_basis: 0` leftover) is an ancestor of the pinned SDK v0.846.0 and NOT of v0.835.0 — `git merge-base --is-ancestor` returns 0 then 1, the control that proves the ancestry test discriminates rather than always agreeing. Re-derived on the pin, not on the v0.834.0 the old note measured: `native editor advertises proportional host-corner resizing` passes 70/70 against v0.846.0, and it now asserts the body viewport at 531±3 with `content_size().height > bounds().height` and a non-zero `scroll_y()` after `set_scroll` — the fixed-header-plus-scrolling-body contract this row is about. Census on v0.843.0 for the exact geometry: exactly ONE ScrollView exists in the whole tree, at (427.00,211.50) 486.00x531.00 with a 558.00x1246.00 content extent, and the frozen-atlas derivation (walk up from the `APPEARANCE` label to its ScrollView) resolves to that SAME object, so the two readings that used to disagree now address one element. Distinguishing control on the same instrument and the same tree: 13 boxes still measure in the 46-54px band (e.g. 448.00x51.00 at (427.00,348.50)), so a ~50px height is representable and readable here — 531 is a real change, not a reader artifact or a unit error | Pending in new PKG | Waiting human confirmation only |
...
| SET-6 | No unnecessary scrollbar when content fits | Implemented | THE STATED BLOCKER IS GONE, AND IT WAS NEVER THIS ROW'S REAL ONE. The body ScrollView measures 531px, not 50px (see SET-1), so "rides on the same ScrollView that is currently 50px tall" no longer describes anything. What actually holds this row open is that nothing oracles it. `detect_scrollbar_painters` matches only `VirtualList`/`VirtualGrid`, so it is structurally BLIND to the widget this row is about: handed a deliberately overflowing `ScrollView` it returns 0 findings, while its shipped control fires on a `VirtualList` — which is exactly why the blindness has read as coverage. `ScrollView::bar_opacity()` is a false oracle in the other direction: it reads 1.000 on hover for a ScrollView whose content FITS (0.000 idle, so the reading is live, not dead). The behaviour itself is correct by construction — `ScrollView::paint` guards each bar on `max_scroll_{y,x}() > 0`, so a fitting viewport paints no track — but that is a source reading, not a test. And the forbidden condition cannot arise on the live settings surface at all: 1246px of content in a 531px viewport means a bar there is necessary, never unnecessary. Closing this row needs a synthetic fitting ScrollView plus a pixel check, or a public `max_scroll` accessor | Pending in new PKG | BLOCKED: unoracled (detector blind to `ScrollView`; `bar_opacity` false-positive), NOT blocked on geometry |
